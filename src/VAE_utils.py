...
def gauss_cross_entropy(mu1, var1, mu2, var2):
    ...
    Args:
        ...
        var1 (torch.Tensor): Variance of the first Gaussian.
        mu2 (torch.Tensor): Mean of the second Gaussian.
        var2 (torch.Tensor): Variance of the second Gaussian.
    """
...
def gmm_fit(data: np.ndarray, mode_coeff=0.6, min_thres=0.3):
    """
    Args:
        data (np.ndarray): Input data to fit the GMM.
        mode_coeff (float): Coefficient for mode calculation.
        min_thres (float): Minimum threshold for the result.
